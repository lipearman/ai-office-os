"""Paper trading — simulated fills (fee 0.25%), positions, journal, stats.
...
def fill_open(entry_price: float, size_thb: float) -> dict:
    """Open a long: qty after entry fee."""
    qty = (size_thb * (1 - FEE)) / entry_price
    return {"qty": qty, "fee_pct": FEE}


def close_pnl(entry_price: float, exit_price: float, size_thb: float, qty: float) -> dict:
    """Realized PnL (THB + %) net of round-trip fee."""
    proceeds = qty * exit_price * (1 - FEE)        # exit fee on sale
